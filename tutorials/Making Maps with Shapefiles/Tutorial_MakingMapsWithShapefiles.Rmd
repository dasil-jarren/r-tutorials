...
# Set working directory to Shiny Grinnell server
# setwd("~/graphics2/Tutorials/Maps_Shapefiles")
...
# Load necessary data
TerrorismData_url <- "https://raw.githubusercontent.com/dasil-jarren/r-tutorials/master/tutorials/Making%20Maps%20with%20Shapefiles/data/terrorismData.csv"
TerrorismData <- read_csv(TerrorismData_url)
...
Welcome to the **Making Maps with Shapefiles** R Tutorial.  This is a continuation of the **Introduction to Creating Maps with ggplot2** R Tutorial that involves further exploration of data, analysis, and visualization in the geographical context.

Before working through this activity it is helpful to have some familiarity with the `ggplot2` package and to complete the **Introduction to Creating Maps with ggplot2** R Tutorial.  

## Setup

### Libraries & Dataset

This tutorial is set up so that you do not have to load in libraries or data.  It is all done for you behind the scenes!  However, if you are working in R or RStudio, you can follow the code below to load the following packages and dataset.
...
In the previous **Introduction to Creating Maps with ggplot2**, we used a simplistic `.csv` file that contained latitude, longitude and group information. However, in most cases, geographic information for maps is stored in a more complex format, commonly referred to as a **shapefile**. 

A shapefile consists of several component files. Not all components are needed, but each shapefile must have at least the following **three** component files:
...
Together with the `TerrorismData` from the [Global Terrorism Database](https://www.start.umd.edu/gtd/), we will be working with a unique filetype: a **shapefile**.  The shapefile includes world's state boundaries that were downloaded from the [Natural Earth website](http//www.naturalearthdata.com/download/50m/cultural/ne_50m_admin_0_countries.zip).

To better understand the available data, we should perform the following activities:

1. Look at a few rows of `TerrorismData` using the `head()` and `tail()` functions.

```{r tab2code1, exercise = TRUE}
head(TerrorismData)
tail(TerrorismData)
```

2. Look at the structure of the `TerrorismData` using the `str()` function.

```{r tab2code2, exercise = TRUE}
str(TerrorismData)
```

3. Look at the structure of the `Worldshapes` shapefile.

```{r tab2code3, exercise = TRUE}
str(Worldshapes, max.level = 2)
```

### Remarks

* The `readShapeSpatial()` function from the `maptools` package allows us to load all component files simultaneously.

* The `str()` function allows us to see that the `Worldshapes` object is of the class `SpatialPolygonsDataFrame`. This means that R is representing this shapefile as a special object consisting of five slots of geographic data. The first slot (and the most relevant to us) is the data slot, which contains a data frame representing the actual data adjoined to the geometries. Similar to how we access a column in a data frame with the `$` infix, we can also access a slot in a shapefile with the `@` infix.
...
```{r tab3code1, exercise = TRUE, message = FALSE, warning = FALSE}
Worldshapes_tidied <- tidy(Worldshapes)
...
<sub>
^1^ Severity is defined in our dataset as the natural log of a weighted sum of the number of people killed and wounded: 

$$
2 \cdot \log{(4 \cdot killed + wounded)}
$$  
</sub>
...
```{r tab3code4, exercise = TRUE}
Worldshapes1 <- mutate(Worldshapes@data, id=as.character(0:240))
Worldshapes2 <- left_join(Worldshapes_tidied, Worldshapes1, by = "id")
